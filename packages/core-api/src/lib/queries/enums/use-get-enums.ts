...
export const getEnums = async (params: GetEnumsParams): Promise<ApiCollectionResponse<Enum>> => {
    return await maasApi.enums.getEnums(params);
};

export const useGetEnums = (
    params: GetEnumsParams,
    options?: Omit<UseQueryOptions<ApiCollectionResponse<Enum>, ApiError>, 'queryKey'>
) =>
    useQuery({
