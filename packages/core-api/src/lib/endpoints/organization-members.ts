...
export class OrganizationMembersEndpoint {
    ...
    async getOrganizationMembers(
        ...
        const { fields, sort, offset = 0, limit = 20, filters } = params;
        return this.client.getCollection<OrganizationMember>(
            `/api/v1/organizations/${filters?.organizationId}/members`,
            fields,
            {
                offset,
                limit,
    ...
    async inviteMember(organizationId: string, invitation: InviteOrganizationMember): Promise<OrganizationMember> {
        return this.client.post<OrganizationMember>(`/api/v1/organizations/${organizationId}/members`, invitation);
    }

    async removeMember(organizationId: string, memberId: string): Promise<void> {
        return this.client.delete<void>(`/api/v1/organizations/${organizationId}/members/${memberId}`);
    }

    async suspendMember(organizationId: string, memberId: string): Promise<OrganizationMember> {
    ...
    async unsuspendMember(organizationId: string, memberId: string): Promise<OrganizationMember> {
