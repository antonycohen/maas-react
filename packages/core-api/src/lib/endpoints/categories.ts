import { Category, CreateCategory, UpdateCategory } from '@maas/core-api-models';
...
export class CategoriesEndpoint {
    ...
    async getCategory(params: GetQueryByIdParams<Category>): Promise<Category> {
        return this.client.getById<Category>(`${BASE_PATH}/${params.id}`, params.fields);
    }
     ...
     */
    async patchCategory(categoryId: string, data: Partial<UpdateCategory>): Promise<Category> {
