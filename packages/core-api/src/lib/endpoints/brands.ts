import { Brand, CreateBrand, UpdateBrand } from '@maas/core-api-models';
...
export class BrandsEndpoint {
    ...
    async getBrand(params: GetQueryByIdParams<Brand>): Promise<Brand> {
        return this.client.getById<Brand>(`${BASE_PATH}/${params.id}`, params.fields);
    }
     ...
     */
    async patchBrand(brandId: string, data: Partial<UpdateBrand>): Promise<Brand> {
