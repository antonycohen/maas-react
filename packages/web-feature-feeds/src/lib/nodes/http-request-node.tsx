            ...
            <div className="space-y-3 p-3">
                ...
                <div>
                    ...
                    <select
                        ...
                        onChange={(e) =>
                            onChange({
                                method: (e.currentTarget as HTMLSelectElement).value as HttpRequestData['method'],
                            })
                        }
                    >
                    ...
                    {data.headers && data.headers.length > 0 && (
                        <div className="max-h-32 space-y-1 overflow-y-auto">
                            {data.headers.map((header, index) => (
                                <div key={header.name || index} className="flex items-center gap-1">
                                    <input
                                        type="text"
                                        placeholder="name"
                                        className="min-w-0 flex-1 rounded border border-slate-200 px-1.5 py-0.5 text-xs outline-none focus:border-slate-400"
                                        value={header.name}
                                    ...
                                    <button
                                        ...
                                        className="flex h-5 w-5 flex-shrink-0 items-center justify-center text-sm font-bold text-red-500 hover:text-red-700"
                                    >
                                        ×
                                    </button>
                                </div>
                ...
                {showBody && (
                    <div>
                        ...
                        <textarea
                            ...
                            className="min-h-[80px] w-full resize-y rounded-lg border border-slate-300 px-2 py-1 text-sm outline-none placeholder:text-slate-400 focus:border-slate-400 focus:ring-0"
                            value={data.body || ''}
