...
export function ControlledTokenInput<TForm extends FieldValues, TValue = string>(
    ...
    const handleRemove = useCallback(
        (index: number) => {
            ...
            field.onChange((updated.length > 0 ? updated : null) as FieldPathValue<TForm, FieldPath<TForm>>);
            setSelectedTokenIndex(null);
            ...
            switch (e.key) {
                ...
                case 'ArrowDown':
                    ...
                    if (source && isOpen && currentFilteredOptions.length > 0) {
                        ...
                        setHighlightedOptionIndex((prev) => (prev < currentFilteredOptions.length - 1 ? prev + 1 : 0));
                ...
                case 'ArrowUp':
                    ...
                    if (source && isOpen && currentFilteredOptions.length > 0) {
                        ...
                        setHighlightedOptionIndex((prev) => (prev > 0 ? prev - 1 : currentFilteredOptions.length - 1));
                    }
                    break;
    ...
    const containerContent = (
        <div
            ref={containerRef}
            className={cn(
                'bg-background flex min-h-[44px] cursor-text flex-wrap items-center gap-1 rounded-md border p-3',
        ...
        >
            {tokens.map((token, index) => (
                <button
                    ...
                    className={cn(
                        'bg-background flex h-8 items-center gap-1 rounded-full border px-2 pr-1 text-sm transition-colors',
                        'focus:ring-ring focus:ring-2 focus:ring-offset-1 focus:outline-none',
                        selectedTokenIndex === index ? 'border-ring bg-accent' : 'border-border hover:bg-accent/50'
                    )}
            ...
            <Input
                ...
                className="h-8 min-w-[120px] flex-1 border-0 p-0 px-1 shadow-none focus-visible:ring-0"
                aria-describedby={tokens.length > 0 ? `${id}-tokens-hint` : undefined}
            />
            {tokens.length > 0 && (
                <span id={`${id}-tokens-hint`} className="sr-only">
                    {tokens.length} item{tokens.length !== 1 ? 's' : ''} selected. Use arrow keys to navigate, Backspace
                    or Delete to remove.
                </span>
            )}
        </div>
