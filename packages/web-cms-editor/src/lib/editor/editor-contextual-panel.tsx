            ...
            {!selectedBlockContent ? (
                ...
                >
                    <div className="flex items-center gap-x-2.5">
                        <span className="text-foreground [&>svg]:h-4 [&>svg]:w-4">{selectedPlugin.icon}</span>
                        <span className="text-foreground text-sm font-bold">{selectedPlugin.displayName}</span>
                    </div>
                    {selectedPlugin.inputsSections
                        .filter(
                            (section) =>
                                !section.visibilityCondition || section.visibilityCondition(selectedBlockContent.data)
                        )
                        .map((section) => (
                            <div
                                key={section.name}
                                className={cn('editor-contextual-panel-list flex flex-col gap-y-4 rounded-lg', {
                                    'border-border border p-4': section.hasBorder,
                                })}
                            >
                                {section.inputs.map((input) => (
                                    <EditorPluginInput key={input.label} input={input} />
                                ))}
                            </div>
                        ))}
                </div>
