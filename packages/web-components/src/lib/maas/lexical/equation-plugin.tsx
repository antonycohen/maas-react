...
const LATEX_COMMANDS: { command: string; example: string; description: string }[] = [
  ...
  { command: "\\begin{pmatrix}", example: "\\begin{pmatrix} a & b \\\\ c & d \\end{pmatrix}", description: "Parenthesis matrix" },
  { command: "\\begin{bmatrix}", example: "\\begin{bmatrix} a & b \\\\ c & d \\end{bmatrix}", description: "Bracket matrix" },
  { command: "\\begin{vmatrix}", example: "\\begin{vmatrix} a & b \\\\ c & d \\end{vmatrix}", description: "Determinant" },
  // Functions
...
export function EquationDialog({ open, onOpenChange }: EquationDialogProps) {
  ...
  useEffect(() => {
    if (autocompleteRef.current && showAutocomplete) {
      const selectedEl = autocompleteRef.current.querySelector(
  ...
  const insertSuggestion = useCallback(
    (suggestion: (typeof LATEX_COMMANDS)[0]) => {
      ...
      if (match) {
        ...
        const newText =
          textBeforeCursor.slice(0, commandStart) +
          suggestion.example +
          textAfterCursor;
        setEquation(newText);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter a LaTeX equation. Type <code className="bg-muted px-1 rounded">\</code> to see available commands.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4 py-4">
          ...
          <div className="relative">
            ...
            {showAutocomplete && filteredSuggestions.length > 0 && (
              <div
                ref={autocompleteRef}
                className="absolute left-0 right-0 top-full z-50 mt-1 max-h-[240px] overflow-y-auto rounded-md border bg-popover shadow-lg"
              >
