...
export const ColorPicker = ({ value, defaultValue = '#000000', onChange, className, ...props }: ColorPickerProps) => {
    ...
    const defaultColor = defaultValue ? Color(defaultValue) : Color('#000000');
    const initialColor = selectedColor ?? defaultColor;
...
export const ColorPickerSelection = memo(({ className, ...props }: ColorPickerSelectionProps) => {
    ...
    // Compute cursor position from saturation/lightness
    const positionX = useMemo(() => Math.max(0, Math.min(1, saturation / 100)), [saturation]);
    ...
    return (
        <div className="relative">
            <Input
                ...
                type="text"
                {...props}
                className={cn('bg-secondary h-8 w-[3.25rem] rounded-l-none px-2 text-xs shadow-none', className)}
            />
...
export const ColorPickerFormat = ({ className, ...props }: ColorPickerFormatProps) => {
    ...
    const computedHex = color.hex();
    const [localHex, setLocalHex] = useState<string | null>(null);
    const isFocusedRef = useRef(false);

    // Use local value when focused/editing, otherwise use computed value
    const hexInput = localHex !== null ? localHex : computedHex;
    ...
    const handleHexChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const parsedColor = Color(value.startsWith('#') ? value : `#${value}`);
            const [h, s, l] = parsedColor.hsl().array();
            setHue(h);
            setSaturation(s);
            setLightness(l);
            setAlpha(parsedColor.alpha() * 100);
        } catch {
            // Invalid color, keep the input value but don't update picker
        }
    };

    const handleFocus = () => {
        isFocusedRef.current = true;
        setLocalHex(computedHex);
    };
